  ...
  search: function (searchObject, resolve, reject) {
    fs.readFile(FILE_NAME, function (err, data) {
      if (err) {
        ...
        if (searchObject) {
          pies = pies.filter(
            /* filter puts all objects that match the filter function 
               Here, if the searched object has an ID, then we return the ones
               that have a matching ID and whose name contains 
                the searchObject name. Otherwise, we return the biggest matching 
                list. */
            (p) =>
